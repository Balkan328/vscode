...
while True:
    # Prendre une image de la cam�ra
    ret, frame = cap.read()

    # Trouver une balle rouge sur l'image
    circles, hsv, mask, gray, gray_blurred = detect_red_ball_on_frame(frame)

    # Mettre un cercle vert autour de la ball trouv�e pour debug
    draw_circles_on_frame(frame.copy(), circles)

    # Montrer d'autres les �tapes de la detection de la balle rouge pour debug
    if hsv is not None:
        cv2.imshow("HSV", hsv)
    if gray is not None:
...
#        cv2.imshow("Mask", mask)
    ...
    # Si le script est lanc� en ligne de commande, on peut quitter avec 'q'
